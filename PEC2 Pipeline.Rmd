...
Se guardaron los *counts* filtrados y se exportaron al repositorio *Github*.

```{r}
write.csv(counts.guardados,file="Counts.filtrados.csv")
```
...
```{r}
Transformacion_Voom <- voom(y,mat_diseño,plot = TRUE)
names(Transformacion_Voom)
```

Se guardaron los *counts* normalizados y transformados, y se exportaron al repositorio *Github*.

```{r}
write.csv(Transformacion_Voom,file="Counts.Normalizados.csv")
```
...
```{r}
boxplot(Transformacion_Voom$E, xlab="1: ELI, 2:NIT, 3:SFI", ylab="Log2 CPM", las=2, names = c(targets$Group[1:30]))
abline(h=median(Transformacion_Voom$E),col="blue")
```
